...
FON = CTkImage(Image.open("fon.png"),size=(350,400))#fon 350,400
CONF = CTkImage(Image.open("conf.png"),size=(20,20))#conf 20,20
USER = CTkImage(Image.open("user.png"),size=(20,20))#user 20,20
ICONS = [] #user icons 40,50
for i in range(7):
...
class MyBtn(CTkButton):
    def __init__(self, master, width = 140, height = 40, text = "CTkbutton", image = None,command = None):
        ...
        super().__init__(master,      width=width ,height=height, corner_radius=20,
                         fg_color=PURPULE, hover_color=MEDIUMBLUE,text=text,
                         font=("Arial",16,"bold"),
                         text_color="white",
...
class Mess(CTkLabel):
    def __init__(self,master,urer,icon,text,anchor):
        ...
        super().__init__(master = master, fg_color="#4C474B",text_color="white",
                         font=("Arial",16,"bold"),image=icon,compound="left",corner_radius=20,padx=10,
    ...
    def send_mess(self):
        ...
        try:
            ...
            Mess(self.all_mess, self.USER, self.ICON, mess, "w")
                ...
                Mess(self.all_mess, "server", 0, "Goodbye")
                self.after(100, self.close)
app = App()
app.mainloop()